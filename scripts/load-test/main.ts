...
const main = async () => {
  ...
  while (numWalletsFunded < numWallets) {
    const numWalletsToFund = Math.min(maxWalletsPerDistribution, numWallets - numWalletsFunded)
    const fundingAmount = totalEthAllocation.mul(numWalletsToFund).div(numWallets)
    const walletsToFund = wallets.slice(numWalletsFunded, numWalletsFunded + numWalletsToFund).map((wallet) => {
    ...
    console.log(`distributed ${ethers.utils.formatEther(fundingAmount)} ETH to ${walletsToFund}`)
    console.log(`https://kovan-optimistic.etherscan.io/tx/${l2DistributionResult.hash}`)
    await l2DistributionResult.wait()
    numWalletsFunded += numWalletsToFund
  }
  ...
  try {
    ...
    await Promise.all(wallets.map(async (wallet) => {
      ...
      if (l2RefundAmount.gt(0)) {
        ...
        console.log(`returned ${ethers.utils.formatEther(l2RefundAmount)} ETH from ${l2Wallet.address}`)
        console.log(`https://kovan-optimistic.etherscan.io/tx/${l2RefundResult.hash}`)
      }
    }))
    ...
    console.log(`https://kovan-optimistic.etherscan.io/tx/${l2WithdrawResult.hash}`)
  }
