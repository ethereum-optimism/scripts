...
const verifier = new JsonRpcProvider({
  url: process.env.VERIFIER_ENDPOINT,
  timeout: 100000000,
});
const sequencer = new JsonRpcProvider({
  url: process.env.SEQUENCER_ENDPOINT,
  timeout: 100000000,
});
...
async function main() {
  ...
  const vfileSafeURL = process.env.VERIFIER_ENDPOINT.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  const sfileSafeURL = process.env.SEQUENCER_ENDPOINT.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  const fileName = `${vfileSafeURL}_v_${sfileSafeURL}_block${blockNum}_diff.json`
  fs.writeFileSync(fileName, JSON.stringify(differences, null, 2));
...
async function getDiff(blockNum, addressMapping) {
  ...
  while(true) {
    try {
      vdump = await verifier.send("debug_dumpBlock", [`0x${blockNum.toString(16)}`]);
      break
    } catch (error) {
      console.log('Error while fetching verifier dump', error)
      console.log('retrying...')
    }
  }
  console.log('got verifier dump')
  while(true) {
    try {
      sdump = await sequencer.send("debug_dumpBlock", [`0x${blockNum.toString(16)}`]);
      break
    } catch (error) {
      console.log('Error while fetching sequencer dump', error)
      console.log('retrying...')
    }
  }
  console.log('got sequencer dump')
  ...
  if (tempDiff) {
    ...
    for (const diffItem of tempDiff) {
      ...
      // if (
        // !(diffItem.path[1] === "OVM_L2CrossDomainMessenger")
...
async function binarySearch() {
  ...
  while (startBlock + 1 !== endBlock) {
    ...
    if (
      ...
      differences.find(
        (diffItem) => diffItem.path[1] === "Synthetix" && diffItem.path[2] === "storage"
      )
    ) {
